...
// Obtener datos
try {
    $entradas  = $controller->entradas($fechaInicio, $fechaFin, $usuarioId, $productoId, $numero);
} catch (Exception $ex) {
    $entradas = [];
    $error = $ex->getMessage();
}
...
<style>
  /* 🔧 Ajuste de ancho y estilo general del reporte */
  .container {
    max-width: 95%;
    margin: auto;
  }
...
<div class="container mt-4 ">
  <h2>📥 Reporte de Entradas (Remitos de Ingreso)</h2>
...
function imprimirReporte() {
  ...
  ventana.document.write(`
    <html>
      ...
      <body>
        <h3>📥 Reporte de Entradas — ${new Date().toLocaleDateString()}</h3>
        ${area}
      </body>
    </html>
  `);
